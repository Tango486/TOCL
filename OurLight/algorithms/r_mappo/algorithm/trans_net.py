...
from OurLight.algorithms.r_mappo.algorithm.frap_net import *
...
class TransformerEncoderModel(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        x = self.transformer_encoder(x_pos, src_mask)
    ...
    def compute_score(self, src, src_mask=None):
        T_batch, num_agents = src.shape[0], src.shape[1]
        phase_emb = self._phase_embedding(src.reshape(T_batch*num_agents, -1))
        x = phase_emb.reshape(T_batch, num_agents, -1)
        ...
        x_pos = self.ps_encoding(x)
        x_pos_ = x_pos.permute(1, 0, 2)

        src_mask = src_mask.repeat(self.args.trans_heads, 1, 1)
        h, attn = self.transformer_encoder(x_pos_, src_mask, output_att=True)

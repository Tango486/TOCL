...
from OurLight.envs.sumo_files_marl.env.intersection import Intersection
import re
    ...
    def _init_sim(self, sumocfg_file, seed, episode_length_time, gui=False):
        ...
        if self.is_record:
            ...
            command += ['--tripinfo-output',
                        self.output_path + '/' + '{}_{}_{}_{}_trip.xml'.format(self.name, re.split(r"[\\/]+", sumocfg_file)[-1], self.thread_id, self.p),
    ...
    def _do_action(self, action):
        if self.action_type == 'select_phase':
            for tl, a in action.items():
                assert a / 2 not in self._crosses[tl].unava_index, "tl:{}-current_action:{}-unava_index:{}".format(tl, a, self._crosses[tl].unava_index)
...
if __name__ == '__main__':
    config = {
        "name": "colight",
        "agent": "",
        "sumocfg_file": "OurLight/envs/sumo_files_marl/scenarios/tidal/tidal.sumocfg",
    ...
    for i in range(100):
        ...
        for tl in all_tl:
            ...
            print(env._crosses[tl].green_phases)
